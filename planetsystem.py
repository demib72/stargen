...
class PlanetSystem:
    ...
    def make_content_list(self):
        ...
        if self.gasarrangement is not 'None':
    ...
    def place_gas_giants(self):
        ...
        if self.gasarrangement is 'Epistellar':
        ...
        elif self.gasarrangement is 'Eccentric':
            for stellar_orbit in small_orbits:
            ...
            for stellar_orbit in large_orbits:
                if self.roller.roll_dice(3, 0) <= 14:
                    self.orbitcontents[stellar_orbit] = GasGiant(self.parentstar,
                                                                   stellar_orbit, self.gas_giant_bonus(stellar_orbit))
        elif self.gasarrangement is 'Conventional':
            for stellar_orbit in large_orbits:
    ...
    def fill_orbits(self):
        ...
        roll_orbits = [orb for orb in self.orbitarray if self.orbitcontents[orb] is None]
        roll_orbits.sort()
        ...
        orc = {k: v for k, v in self.orbitcontents.items() if v is not None}
    ...
    def orbit_fill_modifier(self, orbitindex):
        ...
        if orbitindex is not len(orbits) - 1:
            if self.orbitcontents[orbits[orbitindex + 1]] is not None:
                if self.orbitcontents[orbits[orbitindex + 1]].type() is "Gas Giant":
                    modifier -= 6
